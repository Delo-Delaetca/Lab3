...
LLM_MODEL=Модель_ЛЛМ (в моём случае openai/gpt-oss-120b:free)
```

В проекте используется streamlit, благодаря которому вы увидете сам сайт с проектом. С левой стороны будет видна кнопка загрузки файлов, там можете выбрать нужный файл. Ниже этой кнопки есть подсказка "Как использовать", в которой написано, что можете сделать. Ну и после всего этого вам надо будет написть в терминале: 
...
Потом начал делать самого агента. До этого я делал LLM с шаблоном ответа, но всё же смог продвинуться дальше и получились файлы: agent_memory.py и llm_agent.py.  
